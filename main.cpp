...
//    if (cameraParams.viewSelectionSubset.begin() + maximum_view >= cameraParams.viewSelectionSubset.end())
//    {
//      // Less cameras then requested -> dont delete any
//    }
//    else
//    {
//      cameraParams.viewSelectionSubset
//                  .erase(cameraParams.viewSelectionSubset.begin() + maximum_view,
                    ...
                    )
{
  ...
  if (!inputFiles.gt_filename.empty())
  {
    ...
    cout
        << "gt: "
        << groundTruthDisp.rows
        << " "
        << groundTruthDisp.cols
        << " "
        << groundTruthDisp.channels()
        << " "
        << groundTruthDisp.depth()
        << endl;
  ...
  cout
      << "Range of Minimum/Maximum depth is: "
      << algParams.min_disparity
      << " "
      << algParams.max_disparity
  ...
  testImg_display.copyTo(planes_display(Rect(cols - testImg_display.cols,
                                             0,
                                             testImg_display.cols,
                                             testImg_display.rows)));
  ...
  for (size_t i = 0; i < (size_t) algParams.num_img_processed; i++)
  {
    ...
    CameraParameters
        camParamsNotTransformed = getCameraParameters(*(gs->cameras), inputFiles, algParams.cam_scale, false);
    char plyFile[256];
