...
// CHECKED
__device__ FORCEINLINE void get3Dpoint_cu(float4 *__restrict__ ptX,
                                          const Camera_cu &cam,
                                          const int2 &p,
                                          const float &depth)
{
...
// CHECKED
__device__ FORCEINLINE void getCorrespondingPoint_cu(const int2 &p,
                                                     const float *__restrict__ H,
                                                     float4 *__restrict__ ptf)
{
...
template<typename T>
      ...
      const float &w)
  {
    ...
    /*pt_r.y < rows ) */
    {
      ...
      const float
          colDiff = l1_norm(tex2D<T>(l, pt_l.x + 0.5f, pt_l.y + 0.5f) - tex2D<T>(r, pt_r.x + 0.5f, pt_r.y + 0.5f));
      const float colDis = fminf(colDiff, tau_color);
      ...
      const AlgorithmParameters &algParam)
  {
    ...
    for (int i = -hRad; i < hRad + 1; i += WIN_INCREMENT)
    {
      for (int j = -vRad; j < vRad + 1; j += WIN_INCREMENT)
      {
        ...
        const float
            w = weight_cu<T>(tex2D<T>(l, pt_l.x + 0.5f, pt_l.y + 0.5f), tex2D<T>(l, p.x + 0.5f, p.y + 0.5f), gamma);
...

    {
      ...
      for (int i = -hRad; i < hRad + 1; i += WIN_INCREMENT)
      {
        for (int j = -vRad; j < vRad + 1; j += WIN_INCREMENT)
        {
          ...
          cost =
              cost + pmCostComputation_shared<T>(l, tile_left, r, leftValue, pI, pt, tau_color, tau_gradient, alpha, w);
          //weightSum = weightSum + w;
        }
      }
      return cost;
      ...
      const int point)
  {
    ...
    for (int i = 0; i < camParams.viewSelectionSubsetNumber; i++)
    {
      ...
      /*if ( idxCurr != REFERENCE ) */
      {
        ...
        {
          c = MAXCOST;
        } // in order to not get an overflow when accumulating
        costVector[i] = c;
        cost_count++;
      }
    }
...
//  rework: This function creates something cuda can not handle
...
template<typename T>
  __device__ FORCEINLINE void gipuma_checkerboard_spatialPropFar_cu(GlobalState &gs,
                                                                    int2 p,
                                                                    const int2 tile_offset,
                                                                    int iter)
  {
...
template<typename T>
  __device__ FORCEINLINE void gipuma_checkerboard_spatialPropClose_cu(GlobalState &gs,
                                                                      int2 p,
                                                                      const int2 tile_offset,
                                                                      int iter)
  {
...
template<typename T>
  __device__ FORCEINLINE void gipuma_checkerboard_planeRefinement_cu(GlobalState &gs,
                                                                     int2 p,
                                                                     const int2 tile_offset,
                                                                     int iter)
  {
...
template<typename T>
  __global__ void gipuma_black_planeRefine_cu(GlobalState &gs, int iter)
  {
    ...
    tile_offset.x = blockIdx.x * blockDim.x - WIN_RADIUS_W;
    tile_offset.y = 2.0 * blockIdx.y * blockDim.y - WIN_RADIUS_H;
//    printf("%d, %d to %d, %d\n",p.x,p.y, tile_offset.x, tile_offset.y);
...
template<typename T>
  void gipuma(GlobalState &gs)
  {
    ...
    for (int it = 0; it < maxiter; it++)
    {
      ...
      printf("grid: %d,%d,%d , block: %d,%d,%d , shared: %d\n",grid_size.x,grid_size.y,grid_size.z,block_size.x,block_size.y,block_size.z, shared_size_host);
